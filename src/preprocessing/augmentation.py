...
def aug_directory(
    ...
    -----
        ...
        rand_rot (float): random rotation factor
    ...
    for image_path in src_dir.iterdir():
        if image_path.suffix.lower() in accepted_file_types:
             with Image.open(image_path) as img, Image.open(src_dir_lab / f"{Path(image_path).stem}.png") as img_lab: # RT added to open label image (***hard wired .png extension at the moment***)
                img = img.convert("RGB")  
                img_lab = img_lab.convert("L") # Open in grayscale mode to limit impact of color jitter on masks (need to investigate as may be better to have a separate colorjitter function for the images only)
                count += 1
                out_image, out_lab = augment_image(img=img, img_lab = img_lab, col_jit=col_jit, rand_rot=rand_rot, rand_hflip=rand_hflip)   
